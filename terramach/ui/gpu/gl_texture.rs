...
impl Texture {
    pub fn new(
        ...
        unsafe {
            let _gl_guard = gl_context.lock_current();
        ...
        if self.need_preroll || size_changed {
            {
                let texture_size = ISize::new(
                    (size.width as f32 * self.pixel_ratio).ceil() as i32,
                    (size.height as f32 * self.pixel_ratio).ceil() as i32,
                );
                let _gl_guard = self.gl_context.lock_current();
                unsafe {
